 ...
 */
public class Generic_58mm_Printer implements IPrinter {
    ...
    public Generic_58mm_Printer(BluetoothPrinterListener listener, BluetoothDevice device, Context c)
    {
    ...
    private void PrintReceiptDateAndServer(Receipt receipt, int maxCharacterPerLine)
    {
    ...
    private void PrintReceiptNumberAndTable(Receipt receipt,int maxCharacterPerLine)
    {
        if(common.myAppSettings.GetPrintBarcodeFlag()) {
            ...
            if(receipt.server !=null && receipt.server.Name.length()>0) {
                printerDriver.BT_Write("Server: " + receipt.server.Name);
            }
        }
        else {
            if(receipt.receiptNumber.length()>0 && receipt.printReceiptType!= Enum.PrintReceiptType.total)
    ...
    private void PrintReceiptItems(Receipt receipt,int maxCharacterPerLine)
    {
       ...
       /* ArrayList<StoreItem>items = receipt.myCart.GetItems();
        ...
        {
            PrintStoreItem(items.get(i),maxCharacterPerLine);
        }*/
        printerDriver.LF();
    }
    public void OpenCashDrawer()
    {
        printerDriver.OpenDrawer((byte)1,(byte)1,(byte)1);
    }
    private void PrintPromotionAwarded(PromotionAwarded pa, int maxCharacterPerLine,int receiptIndex)
    {
        ...
        if(pa.promotionObject.GetDiscountType()== Enum.DiscountType.cash) {
    ...
    private void PrintReceiptTotalSection(Receipt receipt,int maxCharacterPerLine,Enum.PrintReceiptType printReceiptType)
    {
        ...
        String strPrice = common.Utility.ConvertBigDecimalToCurrencyFormat(receipt.ReturnReceiptFinalTotalAmount());
    ...
    public void PrintReceipt(Receipt receipt)
    {
        ...
        if(receipt.companyProfile.Logo!=null) {
            ...
            PrintLogo(receipt.companyProfile.Logo);
        ...
        PrintReceiptNumberAndTable(receipt, RegularFontMaxCharPerLine);

        PrintReceiptDateAndServer(receipt, RegularFontMaxCharPerLine);
