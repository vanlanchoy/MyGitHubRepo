 ...
 */
public class LinearLayoutWithExpansionAnimation extends LinearLayout {
    ...
    boolean blnAnimationEnd=true;
    int use_this=0;
    public float lastX=0;
    public float lastY=0;

    IOptionPopupAnimationCompleteListener listener;
    public LinearLayoutWithExpansionAnimation(Context context)
    {
        super(context);
        Instantiate();
    }
    public LinearLayoutWithExpansionAnimation(Context context, AttributeSet attrSet)
    {
        super(context,attrSet);
        Instantiate();
    }
    public void SetListener(IOptionPopupAnimationCompleteListener l)
    {
        listener = l;
    }
    ...
    @Override//only triggered onDraw when border applied
    protected void onDraw(Canvas canvas)
    {

        if(getVisibility()==INVISIBLE)setVisibility(VISIBLE);
        final RelativeLayout.LayoutParams rllp=(RelativeLayout.LayoutParams)getLayoutParams();
        if(rllp.height==TARGET_HEIGHT && rllp.width==TARGET_WIDTH){
            if(blnInverseExpansion && !blnAnimationEnd) {
                blnAnimationEnd = true;
        ...
        else
        {
            ...
            if(rllp.height==TARGET_HEIGHT && rllp.width==TARGET_WIDTH) {
                if(listener!=null)listener.ShrinkAnimationCompleted();
